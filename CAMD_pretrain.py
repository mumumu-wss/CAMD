...
import models_CAMD
from CAMD_pretrain_engine import train_one_epoch
...
def get_args_parser():
    ...
    # Model parameters
    parser.add_argument('--model', default='sim_vit_base_patch16', type=str, metavar='MODEL',
    ...
    parser.add_argument('--select_top_k', default=2, type=int, help='cross-scale attention top-k')
    parser.add_argument('--dis_lay', type=int, nargs='+', default=[2, 5, 8, 11], help='distill layers')
    ...
    parser.add_argument('--blr', type=float, default=6.25e-5, metavar='LR',
    ...
    # Dataset parameters
    parser.add_argument('--data_path', default='pretrain_Data/VGGFACE2/vggface2_train_crop',
                        type=str,
                        help='dataset path')  # ImageNet:/datasets01/imagenet_full_size/061417/
    ...
    # data
    parser.add_argument('--crop_min', default=0.08, type=float)
    parser.add_argument('--crop_max', default=0.60, type=float)
    ...
    parser.add_argument('--online_ln', default=False, action='store_true',
                        help='also use frozen LN in online branch')

    parser.add_argument('--loss_type', default='sim')
    parser.add_argument('--neg_weight', default=0.02, type=float)
    parser.add_argument('--xi_1', default=4, type=float)
    parser.add_argument('--xi_2', default=1, type=float)
    parser.add_argument('--beta_dis', default=0.1, type=float)

    parser.add_argument('--with_blockwise_mask', default=True, action='store_true')
    parser.add_argument('--blockwise_num_masking_patches', default=118, type=int)

    parser.add_argument('--distill', default=True,
                        help='use mid distill or not')
    # hyper-parameter
    parser.add_argument('--mm', default=0.995, type=float)
    parser.add_argument('--mmschedule', default='cosine')
...
class DataAugmentationForSIM(object):
    def __init__(self, args):
        self.args = args
        self.random_resized_crop = SingleRandomResizedCrop(args.input_size, scale=(args.crop_min, args.crop_max), interpolation=3)
        self.no_crop = SingleRandomResizedCrop(args.input_size, scale=(1.0, 1.0), interpolation=3)
        ...
        self.format_transform = transforms.Compose([
            ...
            # FRL_VGG-Face2
            transforms.Normalize(
                mean=[0.5231, 0.4044, 0.3489],std=[0.2536, 0.2194, 0.2070])
        ])
    ...
    def __call__(self, image):
        ...
        spatial_image2, i2, j2, h2, w2, W = self.no_crop(spatial_image2)
    ...
    def __repr__(self):
        ...
        repr += "  transform = %s,\n" % str(self.random_resized_crop) + str(self.random_flip) + str(
            self.color_transform1) + str(self.format_transform)
        repr += ")"
        return repr
...
def main(args):
    ...
    if args.loss_type in ['CAMD']:
        print("use DataAugmentationForSIM")
        transform_train = DataAugmentationForSIM(args)
    else:
        print("use NormalDataAumentation")
    ...
    # build model
    model = models_CAMD.__dict__[args.model](norm_pix_loss=args.norm_pix_loss, args=args)
    model.to(device)
