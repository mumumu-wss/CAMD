    ...
    for data_iter_step, data in enumerate(metric_logger.log_every(data_loader, print_freq, header)):
        ...
        if args.loss_type in ['CAMD',]:
            ...
            with torch.cuda.amp.autocast(enabled=(not args.fp32)):
                loss, global_loss , distill_loss, pixel_loss, outputs = model(x1, x2, rel_pos_21, mm, update_mm, mask=mask)
        ...
        loss = loss*args.xi_1 + global_loss*args.xi_2

        if distill_loss is not None:
            if distill_loss > 1.0 :
                loss += 0.01 * distill_loss
            elif distill_loss < 1.0 :
                loss += args.beta_dis * distill_loss
