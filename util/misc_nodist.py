...
class NativeScalerWithGradNormCount:        # 混合精度训练中处理损失缩放和梯度裁剪
    state_dict_key = "amp_scaler"

    def __init__(self, enabled=True, growth_interval=2000):
        self.enabled = enabled  # 指示是否启用混合精度训练
        self._scaler = torch.cuda.amp.GradScaler(
            enabled=enabled, growth_interval=growth_interval)       # 初始化 GradScaler

    def __call__(self, loss, optimizer, clip_grad=None, parameters=None, create_graph=False, update_grad=True):
        self._scaler.scale(loss).backward(create_graph=create_graph)         # 对损失进行缩放并反向传播
        if update_grad:     # 如果需要更新梯度
            if clip_grad is not None:       # 如果需要裁剪梯度
                assert parameters is not None
                self._scaler.unscale_(optimizer)  # unscale the gradients of optimizer's assigned params in-place 对优化器分配的参数梯度进行反缩放（就地操作）
                norm = torch.nn.utils.clip_grad_norm_(parameters, clip_grad)        # 对梯度进行裁剪
            else:
                self._scaler.unscale_(optimizer)
                norm = get_grad_norm_(parameters)       # 获取梯度范数
            self._scaler.step(optimizer)        # 更新优化器
            self._scaler.update()       # 更新缩放器状态
        else:
            norm = None     # 返回梯度范数
        return norm

    def state_dict(self):
        return self._scaler.state_dict()        # 返回缩放器的状态字典

    def load_state_dict(self, state_dict):
        if self.enabled:
            self._scaler.load_state_dict(state_dict)        # 加载状态字典
