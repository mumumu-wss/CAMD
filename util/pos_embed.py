...
def get_2d_sincos_pos_embed_relative(delta_i, delta_j, delta_h, delta_w, relative_flip, flip_delta_j, embed_dim, grid_size, cls_token=False):
    ...
    delta_j = delta_j.to(device)
    delta_h = delta_h.to(device)
    delta_w = delta_w.to(device)
    relative_flip = relative_flip.to(device)
    flip_delta_j = flip_delta_j.to(device)
    ...
    omega = torch.arange(embed_dim//4, dtype=torch.float32, device=device) / (embed_dim/4)
    omega = 1. / (10000**omega)
