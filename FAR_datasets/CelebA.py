...
class CelebA(data.Dataset):
    def __init__(self, part_dir, attr_dir, partition, img_dir, transform, few_shot_ratio=1):
        ...
        # Few-shot: Retain 1% of the data for training
        if partition == '0':  # Assuming '0' is the training partition
            num_samples = len(self.img)
            selected_indices = np.random.choice(num_samples, int(few_shot_ratio * num_samples), replace=False)
            self.img = [self.img[i] for i in selected_indices]
