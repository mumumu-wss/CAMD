...
class RAFDB(data.Dataset):
    def __init__(self, root='/media/jiaren/DataSet/basic/', split='train', transform=None, few_shot_ratio=1):
        super().__init__()
        ...
        if split == 'train':
            random_seed = 1234
            if few_shot_ratio is not None and 0 < few_shot_ratio < 1:
                num_samples = len(self.samples)
                few_shot_size = int(few_shot_ratio * num_samples)

                # Fix the random seed if provided to ensure reproducibility
                if random_seed is not None:
                    np.random.seed(random_seed)

                selected_indices = np.random.choice(num_samples, few_shot_size, replace=False)
